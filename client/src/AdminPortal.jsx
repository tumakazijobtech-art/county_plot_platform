...
function AdminPortal() {
  ...
  const saveManager = async (event) => {
    ...
    if (admin?.email && payload.email === String(admin.email).toLowerCase()) {
      flash('That email is reserved for the primary admin login. Use a different email for this manager.', 'warning')
      return
    }
    try {
      setBusy(true)
